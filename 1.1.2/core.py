...
class core(GUI.mainWin):
    ...
    def setConnect(self):
        ...
        self.editor.open_file.connect(self.open_file)
        self.editor.message.connect(self.status_left.setText)

    def update_status(self, line, col):

        self.status_right_label.setText('行：%d，列:%d    ' % (line+1, col+1))

    def msg(self, msgs):
        self.status_left.setText(msgs)
    def msg_tmp(self, msgs,time):
        self.statusBar.showMessage(msgs,time)
    ...
    def open_file(self, file_name=''):
        ...
        if file_name != '' and type(file_name) == str:
            if not file_name.lower().endswith('.py'):
                if not file_name.lower().endswith('.pyw'):
            ...
            if self.file_name.lower().endswith('.json'):
                self.A_run.setEnabled(False)
                self.set_editor(self.config.lexer_json)
            else:
        ...
        file_name = str(QtWidgets.QFileDialog.getOpenFileName(
            self, '打开..', os.path.join(os.path.expanduser("~"), 'Desktop'), 'Python源文件(*.py);;PythonW源文件(*.pyw);;JSON文件(*.json)')[0]).strip()
        if file_name != '':
            self.file_name = file_name
            with open(self.file_name, 'r', encoding='UTF-8') as f:
                self.editor.setText(f.read())
    ...
    def setWin_show(self):
        ...
        if not os.path.isfile('%s\json\config.json' % sys.path[0]):
            QtWidgets.QMessageBox.critical(
                self, '错误', '无法打开配置文件"%s\json\config.json"！' % sys.path[0])
            return
        self.open_file('%s\json\config.json' % sys.path[0])
    ...
    def run(self):
        ...
        self.debuger = debug.Debug(
            self.os, self.textEdit, self.tmp, self.statusBar)
    ...
    def pydoc(self):
        ...
        if self.os == 'win32':
            ...
            self.statusBar.showMessage('pydoc服务已开启！', 5000)
        ...
        if opinion == 0 and self.editor.text() != '' or is_save == True:
            ...
            if self.file_name == '' or save_as:
                ...
                self.file_name = str(QtWidgets.QFileDialog.getSaveFileName(
                    self, '另存为..' if save_as else '保存', '', 'Python源文件(*.py);;PythonW源文件(*.pyw)')[0]).strip()
