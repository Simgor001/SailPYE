...
class Config(object):
    ...
    def set_lexer(self):
        ...
        '''设置词法分析器'''
        self.lexer_json.setDefaultPaper(
            QtGui.QColor(self.theme['theme']['default'][1]))
        self.lexer_json.setDefaultColor(
            QtGui.QColor(self.theme['theme']['default'][0]))

        for i in range(len(self.lexer_json.styles)):
            styles = self.theme['theme'][self.lexer_json.styles[i]]
            self.lexer_json.setColor(QtGui.QColor(styles[0]), i)
            self.lexer_json.setPaper(QtGui.QColor(styles[1]), i)
            font = QtGui.QFont(styles[2], styles[3])
            font.setBold(styles[4])
            font.setItalic(styles[5])
            self.lexer_json.setFont(font, i)
